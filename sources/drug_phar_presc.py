...
class drug_phar_presc(Base):
    ...
    def run(self):
        ...
        query = 'select distinct event_yr, event_mth from phar_presc '
        ...
        for index, c in cdf.iterrows():
          query = "select * from phar_presc  where  event_yr=%s and event_mth=%s "%(c.event_yr, c.event_mth )
          drug=cassandra.cas2pandas(query)
          df=pd.concat([df, drug])


          df.treat_end_dt= df.treat_end_dt.apply(lambda x:x.date() if not pd.isna(x) else x)
          df['treat_end_dt']=pd.to_datetime(df.treat_end_dt,errors='coerce', infer_datetime_format=True)


          df=df[(pd.isna(df.treat_end_dt)) | (df.treat_end_dt.dt.year >= config.startyear)   ]
          if(len(df)==0):
                continue
          ...
          if len(df)>=config.chunksize or index == cdf.index[-1]:  
            ...
            df['origin_source_name']='phar_presc'
            df['origin_source_value']=df['presc_id']
            
            df.rename(columns={'person_id':'person'}, inplace=True)
            df['type_concept_id']=32838

           
            df['start_timestamp']=pd.to_datetime(df.treat_start_dt.astype(str),errors='coerce', infer_datetime_format=True, utc=True).dt.tz_convert("Europe/Madrid")
            df=self.reviewErrors(df, 'start_timestamp',self.source_module, dropErrors=True)

            df['end_timestamp']=pd.to_datetime(df.treat_end_dt.astype(str),errors='coerce', infer_datetime_format=True, utc=True).dt.tz_convert("Europe/Madrid")
            df=self.reviewErrors(df, 'end_timestamp',self.source_module, dropErrors=True)
            
            
            df['concept']=df.pharactivesubs_cd
            df['route']=df.pharadmroute_cd
            df['quantity']=df.dose_qty_nm
            df['provider']=df.presc_cias_cd
            #TODO Beatriz Hacer mapeo de routas
            
            df['route_source_value']= df['route']
            df['route_concept_id'] = 0
            Droga=Drug(self.caches)
            df=Droga.mapRoutes(df)
            df['days_supply']=(df['end_timestamp']-df['start_timestamp']).dt.days
    ...
    def CodigoNacional2Standard(self, cn):  #lo van a pasar a ATC
        #Pendiente de recibir la vista  si le paso el refer me devuelve el ATC de los que no existen
