...
class drug_vaccines(Base):
    ...
    def run(self):
        ...
        query = 'select distinct event_yr from vaccines '
        ...
        for index, c in cdf.iterrows():
          query = "select * from vaccines  where  event_yr=%s"%(c.event_yr)
          ...
          if len(df)>=config.chunksize or index == cdf.index[-1]:  
            ...
            df['origin_source_name']='vaccines'
            ...
            df['start_timestamp']=pd.to_datetime(df.vac_dt.astype(str),errors='coerce', infer_datetime_format=True, utc=True).dt.tz_convert("Europe/Madrid")
            df=self.reviewErrors(df, 'start_timestamp',self.source_module, dropErrors=True)

            df['end_timestamp']=df['start_timestamp']
            
            
            df['concept']=df.vac_cd
            df['lot_number']=df.lote_cd
            df['route']=None

            
            df=self.mapConcepts(df,vocabularios=['ATC','CVX','vacunas'] )
